    ...
    try:
        ...
        if command == '1':
            ...
            person_image = os.path.join(
                'images', 'person1.jpg')  # Also try person2.jpg
            CompareFaces(person_image, os.path.join('images', 'people.jpg'))
        elif command == '3':
            names = ['Aisha', 'Sama']
            TrainModel('employees_group', 'employees', names)
        elif command == '4':
            RecognizeFaces(os.path.join(
                'images', 'people.jpg'), 'employees_group')
        elif command == '5':
            VerifyFace(os.path.join('images', 'person1.jpg'),
                       'Aisha', 'employees_group')
    ...
    with open(image_file, mode="rb") as image_data:
        ...
        if len(detected_faces) > 0:
            ...
            for face in detected_faces:
                ...
                age = 'age unknown' if 'age' not in detected_attributes.keys() else int(
                ...
                if 'emotion' in detected_attributes:
                    ...
                    for emotion_name in detected_attributes['emotion']:
                        print('   - {}: {}'.format(emotion_name,
                                                   detected_attributes['emotion'][emotion_name]))
...
def CompareFaces(image_1, image_2):
    ...
    with open(image_2, mode="rb") as image_data:
        ...
        similar_faces = face_client.face.find_similar(
            face_id=image_1_face.face_id, face_ids=image_2_face_ids)
        ...
        for face in image_2_faces:
            if face.face_id in similar_face_ids:
                ...
                plt.annotate('Match!', (r.left, r.top + r.height + 15),
    ...
    for person_name in image_folders:
        ...
        for pic in person_pics:
            ...
            face_client.person_group_person.add_face_from_stream(
                group_id, person.person_id, img_stream)
        ...
        if len(recognized_faces) > 0:
            ...
            for face in recognized_faces:
                person_name = face_client.person_group_person.get(
                    group_id, face.candidates[0].person_id).name
                print('-', person_name)
        ...
        if person.name == person_name:
            ...
            with open(person_image, mode="rb") as image_data:
                ...
                if len(faces) > 0:
                    ...
                    verification = face_client.face.verify_face_to_person(
                        face_id, person_id, group_id)
